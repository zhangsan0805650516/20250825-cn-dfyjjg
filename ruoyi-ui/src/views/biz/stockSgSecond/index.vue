  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="sgListList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('姓名')" align="center" prop="name" >
        <template slot-scope="scope">
          {{ scope.row.faMember.name?scope.row.faMember.name:"" }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('手机号')" align="center" prop="mobile" width="100">
        <template slot-scope="scope">
          {{ scope.row.faMember.mobile?scope.row.faMember.mobile.replace(/(\d{3})(\d{4})(\d{4})/, "$1****$3"):"" }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('唯一码')" align="center" prop="weiyima" width="100">
        <template slot-scope="scope">
          {{ scope.row.faMember.weiyima?scope.row.faMember.weiyima:"" }}
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="200px">
        <el-form-item :label="$t('申购用户')">
          <el-select
            ...
            <el-option
              ...
              :key="item.id"
              :label="item.nickname + ' ' + item.weiyima"
              :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item :label="$t('申购时间')" prop="createTime">
          <el-input v-model="form.createTime" :placeholder="$t('请输入申购时间')" />
        </el-form-item>
      </el-form>
...
<script>
import { listStockSgSecond, getStockSgSecond, delStockSgSecond, addStockSgSecond, updateStockSgSecond, submitAllocation, subscriptionBg, transToHold, transOneToHold } from "@/api/biz/stockSgSecond/stockSgSecond";
import {getDailiList, searchMember} from "@/api/biz/member/member";
import { getInfoByName } from "@/api/biz/riskConfig/riskConfig";
...
export default {
  ...
  methods: {
    ...
    getInfoByName(name) {
      getInfoByName({"name":name}).then(res => {
    ...
    getList() {
      this.loading = true;
      listStockSgSecond(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getStockSgSecond(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStockSgSecond(this.form).then(response => {
              this.$modal.msgSuccess(this.$t("成功"));
              this.open = false;
              this.getList();
            });
          } else {
            addStockSgSecond(this.form).then(response => {
              this.$modal.msgSuccess(this.$t("成功"));
              this.open = false;
              this.getList();
            });
